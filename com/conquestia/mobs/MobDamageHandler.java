...
public class MobDamageHandler implements Listener {
    ...
    public MobDamageHandler(JavaPlugin plugin)
    {
        ...
        ftr = (FearTheReaper) Bukkit.getPluginManager().getPlugin("FearTheReaper");
    }
    ...
    public void OnMobDamage(EntityDamageByEntityEvent event)
    {
        ...
        if (event.getEntity() instanceof Player && deadPlayers.contains(event.getEntity()) && SpawnPoint.getClosestAllowed((Player) event.getEntity()).getLocation().distanceSquared(event.getEntity().getLocation()) <= 100) {
            ...
            if (event.getDamager() instanceof Projectile && ((Projectile)event.getDamager()).getShooter() instanceof Monster) {
                ((Projectile)event.getDamager()).getShooter().remove();
            }
        ...
        if (event.getDamager() instanceof Player && deadPlayers.contains(event.getDamager()) && SpawnPoint.getClosestAllowed((Player) event.getDamager()).getLocation().distanceSquared(event.getDamager().getLocation()) <= 100) {
        ...
        if (event.getDamager() instanceof Projectile && ((Projectile)event.getDamager()).getShooter() instanceof Monster) {
            ...
            if (le.getCustomName() != null && le.getCustomName().toLowerCase().contains("lvl"))
            {
                ...
                if (cqm.getMobArena() != null && mobConfig.getConfig().getBoolean("MobArenaWaveLeveling", false) && cqm.getMobArena().getArenaMaster().getArenaAtLocation(event.getDamager().getLocation()) != null)
                {
                    wave = cqm.getMobArena().getArenaMaster().getArenaAtLocation(event.getDamager().getLocation()).getWaveManager().getWaveNumber();
                }
